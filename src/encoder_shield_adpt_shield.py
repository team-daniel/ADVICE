...
class Encoder_DDPG_Adpt_Shield():
    ...
    def make_pairs(self, x_train, y_train):
        ...
        # Generate all combinations of safe and safe
        for i in safe_idxs:
            for j in safe_idxs:
                if i != j:  # Ensure we don't pair an index with itself
                    pairs.append([x_train[i], x_train[j]])
                    labels.append(1)  # Similar pair

        # Generate all combinations of unsafe and unsafe
        for i in unsafe_idxs:
            for j in unsafe_idxs:
                if i != j:  # Ensure we don't pair an index with itself
                    pairs.append([x_train[i], x_train[j]])
                    labels.append(1)  # Similar pair

        # Generate all combinations of safe and unsafe
        for i in safe_idxs:
            for j in unsafe_idxs:
                pairs.append([x_train[i], x_train[j]])
